  ...
  return (
    <div className='h-[100vh] overflow-auto'>
      <div className='h-24'>
                ...
                <h2 className=' ml-4 text-xl font-semibold'>Overall Sentiment</h2>
                </div>
                <div className='flex flex-row'>
      ...
      <h1>Food Feedback Sentiment Analyzer</h1>
      <div className='flex flex-wrap ml-16 mt-4'>
  {foodSentiments.map((item, index) => (
    <div key={index} className={`mr-6 mb-5 h-42 p-6 pl-7 rounded-md bg-blue-900 text-slate-100 `}>
      ...
      <li className={`list-disc flex flex-row`}>Sentiment: <p className={` pl-3 ${item.sentimentLabel === 'Positive' ? 'text-lime-500' : 'text-red-700'}`}>{item.sentimentLabel}</p> </li>
    </div>
  ))}
  <div className={`mr-6 w-48 h-52 p-6 pl-7 rounded-md`}>
    <h2>Overall Average Sentiment</h2>
    <p>Average Sentiment Score (out of 10): {overallSentiment.averageSentiment}</p>
    <p>Sentiment: {overallSentiment.sentimentLabel}</p>
  </div>
</div>
...
FoodFeedbackSentimentAnalyzer.defaultProps = {
  feedbackData: [
    ...
    { food: "pizza", comment: "very special." },
    { food: "pizza", comment: " very bad." },
    { food: "burger", comment: "Juicy and flavorful." },
    { food: "burger", comment: "juicy and tastefull." },
    { food: "sushi", comment: "Fresh and tasty!" },
    { food: "sushi", comment: "Not fresh, disappointed." },
    { food: "pasta", comment: "Rich and creamy sauce." },
    ...
    { food: "rajma", comment: "worthless!" },
    { food: "rajma", comment: "Perfect with steamed rice." },
    { food: "rajma", comment: "Spiced more, dull the flavors." },
